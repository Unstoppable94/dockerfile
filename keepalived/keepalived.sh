...
#ADD cp BY XIEHQ 20170614
cp -f /etc/keepalived/keepalived_e.conf /etc/keepalived/keepalived.conf
...
/bin/sed -i "s/{{VIRTUAL_ROUTER_ID}}/${VIRTUAL_ROUTER_ID}/" /etc/keepalived/keepalived.conf
...
#ADD BY XIEHQ 20170614
/bin/sed -i "s/{{CHECK_SCRIPT}}/${CHECK_SCRIPT}/" /etc/keepalived/keepalived.conf
/bin/sed -i "s/{{PRIORITY}}/${PRIORITY}/" /etc/keepalived/keepalived.conf
/bin/sed -i "s/{{CHECK_SCRIPT}}/${CHECK_SCRIPT}/" /etc/keepalived/keepalived.conf
/bin/sed -i "s/{{STATE}}/${STATE}/" /etc/keepalived/keepalived.conf
#END OF XIEHQ 20170614
...
#xiehq modify VR_ID TO VIRTUAL_ROUTER_ID@20170614
if ! [[ $VIRTUAL_ROUTER_ID =~ ^([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-5][0-5])$ ]]; then
  echo "The VIRTUAL_ROUTER_ID environment variable is null or not a number between 1 and 255, exiting..."
  exit 1
fi
